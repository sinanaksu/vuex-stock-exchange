<template>
<div>
  <div class="brown lighten-5 elevation-1" >
    <v-container>
      <v-row>
        <v-col cols="12" class="text-h5 orange--text text--darken-4">
          <span transition="scale-transition">{{ symbol }}</span>
        </v-col>
      </v-row>
    </v-container>
  </div>
...
export default {
  ...
  computed: {
		...
		...mapState(['pageType','selectSymbol','chartData']),
	},
  methods: {
  ...mapActions(['searchSymbol','getSymbolData']),
  ...mapMutations(['SET_PAGE_TYPE', 'SET_SEARCH_DATA','SET_SEARCH_DATA']),
      changeTime(time){
        console.log(time);
      },
      CandlestickChart(data, {
        ...
        stroke = "#888888", // stroke color for the daily rule
  ...
  watch: {
    chartData () {
      let key = Object.keys(this.chartData)[1];
      this.chartDataFormat = Object.keys(this.chartData[key]).map(date => {
            return {
              open: parseFloat(this.chartData[key][date]["1. open"]),
              high: parseFloat(this.chartData[key][date]["2. high"]),
              low: parseFloat(this.chartData[key][date]["3. low"]),
              close: parseFloat(this.chartData[key][date]["4. close"]),
              volume: parseFloat(this.chartData[key][date]["5. volume"]),
              date: new Date(date)
            }
      });
      this.chart = this.CandlestickChart(this.chartDataFormat)
      this.chartInfo = this.chartData["Meta Data"]
    },
